...
import com.iontorrent.vaadin.utils.AreaInputDialog;
 ...
 */
public class TableWindow extends WindowOpener implements Button.ClickListener,
	...
	public void windowOpened(Window mywindow) {
		...
		coordsel = new CoordSelect(mx + exp.getColOffset(), my
		...
		if (app.getExplorerContext() != null
				&& app.getExplorerContext().getMasks() != null) {
			ExplorerContext maincont = app.getExplorerContext();
			usemask = new MaskSelect("table_mask", " ", "Pick a mask to view",
					maincont, MaskSelect.NONE,
					new Property.ValueChangeListener() {
						...
						public void valueChange(ValueChangeEvent event) {
							...
							if (showmask == null)
								return;
							ArrayList<WellCoordinate> coords = showmask
									.getAllCoordsWithData(5000);
							if (coords != null && coords.size() > 0) {
								WellSelection sel = new WellSelection(coords);
								sel.setTitle("Wells of mask "
										+ showmask.getName());
		...
		btnscores
				.setDescription("Computing the scores could take a few minutes");
		...
		Button btnreads = new Button("Enter read names");
		btnreads.setDescription("Enter or paste a list of read names");
		hor.addComponent(btnreads);
		btnreads.setImmediate(true);
		btnreads.addListener(new ClickListener() {
	...
	public void parseReads() {
		AreaInputDialog input = new AreaInputDialog(mywindow,
				"Enter a list of reads, separated by comma (format row.col or row/col or hash:row:col)", new AreaInputDialog.Recipient() {
	...
	public String getHelpMessage() {
		...
		if (this.exp != null) {
			...
			msg += "<li>Current results dir:" + exp.getResultsDirectory()
					+ "</li>";
	...
	private void startToLoadScores() {
		...
		indicator
				.setDescription("I am loading scores from the BAM and .sff files...");
		...
		protected Void doInBackground() {
			try {
				...
				if (!mask.hasAllBamImages()) {
					app.showMessage("Bam File",
							"I will have to process the bam file<br>This might take a few minutes");
					done = false;
				}
				if (!mask.hasAllSffImages()) {
					done = false;
					app.showMessage("Sff File",
							"I will have to process the sff file<br>This might take a few minutes");
				}
				if (!done) {
...
//						ScoreMaskFlag.Q47LEN, ScoreMaskFlag.INDEL, ScoreMaskFlag.SNR };
	...
	private int addRow(WellContext cont, int row, WellCoordinate well) {
		...
		if (bf == null) {
			...
			Object[] rowdata = new Object[bfflags.length + scoreflags.length
					+ 2];
			...
			for (int i = 0; i < scoreflags.length; i++) {
				...
				rowdata[i + offset] = (int) (well.getScoredata(flag) / flag
						.multiplier());
			}
	...
	private static void err(String msg, Exception ex) {
		Logger.getLogger(TableWindow.class.getName()).log(Level.SEVERE,
