 ...
 */
public class BarcodeMaskImage implements StreamResource.StreamSource {
	...
	public BarcodeMaskImage(ScoreMask mask, ExperimentContext exp,
			ScoreMaskFlag flag, int bucket) {
		this.exp = exp;
		if (exp != null) {
			pan = new ScoreDensityPanel(exp);
			p("Created score density panel, bucket= "+bucket);

			pan.setFlag(flag);
			pan.setExp(exp);
			pan.setMin(0.0);
			
			pan.setScoreMask(mask, flag, bucket, 100.0);
	...
	private static void err(String msg, Exception ex) {
		...
		Logger.getLogger(BarcodeMaskImage.class.getName()).log(Level.SEVERE,
				msg, ex);
	}
	...
	private static void warn(String msg) {
		// system.out.println("ScoreMaskImage: " + msg);
		Logger.getLogger(BarcodeMaskImage.class.getName()).log(Level.WARNING,
				msg);
	}

	private static void p(String msg) {
		// system.out.println("ScoreMaskImage: " + msg);
		Logger.getLogger(BarcodeMaskImage.class.getName()).log(Level.INFO, msg);
	}
}
